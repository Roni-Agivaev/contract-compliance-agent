...
DESCRIPTION = (
    ...
    "5. Outer re-audit loop — once the Editor has produced a corrected contract, the whole agent runs "
    "again over that contract, repeating until the validation gate accepts zero breaches or three "
    "full passes have run. This catches problems a single pass never flagged, because Reflection only "
    "checks the issue list it was given rather than re-reading the entire contract. A single API call "
    "therefore returns a converged contract rather than an intermediate one.\n"
    "If no breaches survive the validation gate on the first pass, the Supervisor autonomously returns "
    "the original contract unchanged with a 'No breaches found' message and never invokes the Editor.\n\n"
